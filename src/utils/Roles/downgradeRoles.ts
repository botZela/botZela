...
export async function downgradeRoles(member: GuildMember) {
	...
	if (year.name === '3A') {
		const yr_fl_new = roles.get(yr_fl.name?.replace('3A', '2A') ?? '');
		const prevYear = roles.get('2A');

		await member.roles.add([prevYear ?? '', yr_fl_new ?? ''].filter((x) => x !== ''));
		await member.roles.remove([year.id, yr_fl.id].filter((x) => x !== ''));
