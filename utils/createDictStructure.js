...
async function createDictStructure(client,guild,format,overwrites = null,category = null){
    let name, rolesList,overwritesList,mycategory,channels,channel,channelArg,type,categoryDict;
    ...
    if (keys[0].toLowerCase() === "category"){
        ...
        try{
            ...
            for (channel of channels){
                await createChannelDict(client,guild,undefined,overwritesList,mycategory);
            }
        } catch(e){
            null;
        }
        ...
        await createChannel(client,guild,name,type,overwritesList,mycategory)
    }
}
